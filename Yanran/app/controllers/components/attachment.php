	...
	*/
	...
	function upload_FS(&$data) {
		...
		if ($this->is_image($filetype)) {
			...
			$this->origin($tmpuploaddir.DS.$filename);
	...
	function origin($tmpfile) {
	    if (!$this->config['origin']) {
	        return false;
	    }
	    $originuploaddir = WWW_ROOT.'attachments'.DS.$this->config['files_dir'].DS.'origin';
	    if (!is_dir($originuploaddir)) {
	        mkdir($originuploaddir, 0755, true);
	    }
	    $ds = (strcmp('\\', DS) == 0) ? '\\\\' : DS;
		$file_name = end(split($ds, $tmpfile));
	    copy($tmpfile, $originuploaddir.DS.$file_name);
	    return true;
	}
			...
			if (file_exists($dstimg)) {
				...
				case 'resizeCropForWidth':
					...
					if ($ratioX < $ratioY) {
						...
						$startY = 0;
						$applyWidth  = $newWidth;
						$applyHeight = round($applyWidth / $oldWidth * $oldHeight);
					} else {
						$startX = 0;
						$startY = 0;
						$oldWidth  = $oldWidth;
