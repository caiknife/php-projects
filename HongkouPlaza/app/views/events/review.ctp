<div id="content">
	<div id="main">
		...
		<div id="right">
			...
			<![endif]-->
			<div class="maySwitch">
                <?php if ($prev_month['available']):?>
                <a class="lastMonth" href="<?php echo $this->Html->url(array('year'=>$prev_month['year'], 'month'=>$prev_month['month']))?>"></a>
                <?php endif;?>
                <?php echo $year?>年<?php echo $month?>月 
                <?php if ($next_month['available']):?>
                <a class="nextMonth" href="<?php echo $this->Html->url(array('year'=>$next_month['year'], 'month'=>$next_month['month']))?>"></a>
                <?php endif;?>
            </div>
			<div class="calendar">
				<ul class="thead">
                    ...
                    <li <?php if(!$day['available']):?>class="notThis"<?php elseif($day['today']):?>class="today"<?php endif;?>>
                        ...
                        <div class="alogo">
                            ...
                            <a _title='<?php echo $this->element('events/related_link', array('event'=>$event))?>' href="<?php echo $this->Html->url(array('action'=>'detail', $event['Event']['id']))?>" title="<?php echo h($event['Event']['title'])?> -/ <?php echo $this->Format->toDate($event['Event']['start_date'])?> - <?php echo $this->Format->toDate($event['Event']['end_date'])?>">
                            	<img src="<?php echo $this->Format->showBrandLogoThumb($event)?>" /><i></i>
                            </a>
                            <?php endforeach;?>
                        </div>
...
<?php echo $this->Html->script('frontend/events/review', false)?>
